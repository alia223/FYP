...
class RepeatBookingTest extends TestCase
{
    ...
    public function testCreateARepeatBookingOnThreeDaysOfTheWeek()
    {
        ...
        $expected_dates_of_booking = ['2021-05-03', '2021-05-04', '2021-05-05', '2021-05-10', '2021-05-11', '2021-05-12', '2021-05-17', '2021-05-18', 
            '2021-05-19', '2021-05-24', '2021-05-25', '2021-05-26', '2021-05-31'];
        for($i = 0;$i < sizeof($expected_dates_of_booking);$i++) {
            $this->assertDatabaseHas('bookings', [ 'parent_id' => Auth::id(), 'booking_date' => $expected_dates_of_booking[$i], 
                'booking_day' => date('N', strtotime($expected_dates_of_booking[$i])), 'start_time' => '15:30:00', 'end_time' => '16:00:00', 'duration' => 30]);
            $this->assertDatabaseHas('booked_pupils', [ 'parent_id' => Auth::id(), 'pupil_id' => $pupil->id, 
                'booking_date' => $expected_dates_of_booking[$i], 'booking_day' => date('N', strtotime($expected_dates_of_booking[$i])), 'start_time' => '15:30:00',  
                'end_time' => '16:00:00']);
        }
    }
    ...
    public function testCreateARepeatBookingForEveryDayOfTheWeek()
    {
        ...
        // from the date selected to the recursive_end_date
        $this->post(route('repeat-bookings.store'), ['parent_id' => Auth::id(), 'date' => '2021-05-01', 'booking_length' => '30', 'pupils' => [$pupil->id], 
            'recursive_end_date' => '2021-05-31', 'recursive_days' => [1,2,3,4,5]]);
        //Check that a 30 minute booking exists every monday, tuesday and wednesday in database from selected date to recursive_end_date
    ...
    public function testRepeatBookingDelete()
    {
        ...
        //Check that these this repeat-booking no longer exists i.e. it has been soft deleted (because user is logged in. Only admin can permanently delete,
        //non-admins can only "mark as deleted")
        for($i = 0;$i < sizeof($expected_dates_of_booking);$i++) {
