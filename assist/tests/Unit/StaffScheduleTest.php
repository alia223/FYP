...
class StaffScheduleTest extends TestCase
{
    ...
    public function testStaffScheduleForOnePupilBookedIn() {
        ...
        $this->post(route('bookings.store'), ['parent_id' => Auth::id(), 'booking_length' => '240', 'date' => date('Y-m-d'), 'pupils' => [$pupil->id]]);
        for($i=0;$i<5;$i++) {
        ...
        $this->post(route('staff-schedule.store', ['day' => date('w')]));
    ...
    public function testStaffScheduleForFifteenPupilsBookedIn() {
        ...
        $this->post(route('bookings.store'), ['parent_id' => Auth::id(), 'booking_length' => '240', 'date' => date('Y-m-d'), 
            'pupils' => [$pupils[0]->id, $pupils[1]->id, $pupils[2]->id, $pupils[3]->id]]);
        $this->post(route('bookings.store'), ['parent_id' => Auth::id(), 'booking_length' => '180', 'date' => date('Y-m-d'), 
            'pupils' => [$pupils[4]->id, $pupils[5]->id]]);
        $this->post(route('bookings.store'), ['parent_id' => Auth::id(), 'booking_length' => '60', 'date' => date('Y-m-d'), 
            'pupils' => [$pupils[6]->id, $pupils[7]->id, $pupils[8]->id, $pupils[9]->id, $pupils[10]->id]]);
        $this->post(route('bookings.store'), ['parent_id' => Auth::id(), 'booking_length' => '30', 'date' => date('Y-m-d'), 
            'pupils' => [$pupils[11]->id]]);
        $this->post(route('bookings.store'), ['parent_id' => Auth::id(), 'booking_length' => '90', 'date' => date('Y-m-d'), 
    ...
    public function testStaffScheduleForThirtyPupilsBookedIn() {
        ...
        $this->post(route('bookings.store'), ['parent_id' => Auth::id(), 'booking_length' => '210', 'date' => date('Y-m-d'), 
            'pupils' => [$pupils[16]->id, $pupils[17]->id]]);
        $this->post(route('bookings.store'), ['parent_id' => Auth::id(), 'booking_length' => '120', 'date' => date('Y-m-d'), 
